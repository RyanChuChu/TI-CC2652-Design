...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        ...
        .def ClearDisplay                       ;clear LCD previous outputs
...
Display:
        ...
        PUSH    {R6, LR}                        ;store link register for return to main loop
...
DoneDisplay:
        ...
        POP     {R6, LR}                        ;restore link register for return to main loop
        BX      LR

; ClearDisplay
;
; Description:       This function clears the LCD display by issuing the LCD "Clear Display"
;                    command. The function waits for any previous GPT0-based delay to
;                    complete before sending the command, and then waits until the LCD is 
;                    ready to accept new instructions.
;
; Operation:         First, the function waits for the previous GPT0 timeout to ensure LCD
;                    timing constraints. Then, it sends the Clear Display command (0x01) to 
;                    the LCD instruction register. Finally, it setups specific delay for LCD
...
;
; Input:             None.
; Output:            Write command 0x01 to DIO[15..8] and clear previous LCD output.
;
; Error Handling:    None.
;
; Algorithms:        None.
; Data Structures:   None.
;
; Registers Changed: R0, R5
; Stack Depth:       6 words
;
; Known Bugs:        None.
; Limitation:        None.
;
; Revision History:  12/12/25   Li-Yu Chu      initial revision
...
InitClearWaitPrevTimeout:
                                                ;wait for previous timeout        
        LDR     R6, [R5, #GPT_RIS_OFF]          ;get timer 0 raw interrupt status
        TEQ     R6, #GPT0_TIMEOUT               ;check if timeout interrupt generated
        BNE     InitClearWaitPrevTimeout        ;if not, wait until timeout
        ;BEQ    WriteClear                      ;if yes, can start to write command

WriteClear:

        MOV32   R4, INSTR_RS                    ;for writing command, RS = 0, passed in through R4
        MOV32   R3, 1                           ;Clear Display command = 1   
        LSL     R3, R3, #DATA_TO_DIO            ;align command to data pins DIO[8..15]
        BL      WriteLCD                        ;write command to LCD

ClearWaitLCD:

        MOV32   R4, NO_DELAY_COUNT              ;for writing command, choose delay for command
        BL      WaitLCDReady                    ;setup specific delay for LCD clear display

DoneClearDisplay:

        POP     {R1 - R4, R6, LR}               ;restore registers
        BX      LR                              ;done so return
